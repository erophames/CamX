...
static inline void
calc_output_stereo (SINC_FILTER *filter, increment_t increment, increment_t start_filter_index, double scale, ARES * output)
{	double		fraction, left [2], right [2], icoeff ;
	...
	output [0] = (ARES)(scale * (left [0] + right [0])) ;
	output [1] = (ARES)(scale * (left [1] + right [1])) ;
} /* calc_output_stereo */
...
static inline void
calc_output_quad (SINC_FILTER *filter, increment_t increment, increment_t start_filter_index, double scale, ARES * output)
{	double		fraction, left [4], right [4], icoeff ;
	...
	output [0] = (ARES)(scale * (left [0] + right [0])) ;
	output [1] = (ARES)(scale * (left [1] + right [1]));
	output [2] = (ARES)(scale * (left [2] + right [2])) ;
	output [3] = (ARES)(scale * (left [3] + right [3])) ;
} /* calc_output_quad */
...
static inline void
calc_output_hex (SINC_FILTER *filter, increment_t increment, increment_t start_filter_index, double scale, ARES * output)
{	double		fraction, left [6], right [6], icoeff ;
	...
	output [2] = (ARES)(scale * (left [2] + right [2]) );
	output [3] = (ARES)(scale * (left [3] + right [3]) );
	output [4] = (ARES)(scale * (left [4] + right [4]) );
	output [5] =(ARES)( scale * (left [5] + right [5])) ;
} /* calc_output_hex */
...
static inline void
calc_output_multi (SINC_FILTER *filter, increment_t increment, increment_t start_filter_index, int channels, double scale, ARES * output)
{	double		fraction, icoeff ;
	...
	do
	{
		...
		{	default :
				ch -- ;
				output [ch] = (ARES)(scale * (left [ch] + right [ch])) ;
			case 7 :
				ch -- ;
				output [ch] = (ARES)(scale * (left [ch] + right [ch])) ;
			case 6 :
...
{	int len = 0 ;
	...
	if  (len < 0 || filter->b_end + len > filter->b_len)
		return SRC_ERR_SINC_PRepair_DATA_BAD_LEN ;
