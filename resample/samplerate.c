...
long
src_callback_read (SRC_STATE *state, double src_ratio, long frames, ARES *data)
{	SRC_PRIVATE	*psrc ;
	SRC_DATA	src_data ;
...
src_strerror (int error)
{
	...
	{	case SRC_ERR_NO_ERROR :
		...
		case SRC_ERR_SINC_PRepair_DATA_BAD_LEN :
...
void
src_short_to_float_array (const short *in, ARES *out, int len)
{
	while (len)
	{	len -- ;
		out [len] = (ARES) (in [len] / (1.0 * 0x8000)) ;
		} ;
...
void
src_float_to_short_array (const ARES *in, short *out, int len)
{	double scaled_value ;
...
void
src_int_to_float_array (const int *in, ARES *out, int len)
{
	while (len)
	{	len -- ;
...
void
src_float_to_int_array (const ARES *in, int *out, int len)
{	double scaled_value ;
