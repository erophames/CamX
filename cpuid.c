...
void map_mname(int family, int model, const char *name, char *m_name)
{

    // Default to name not known
    m_name[0] = '\0';

    if (!strncmp("AuthenticAMD", name, 12)) {
    ...
    else if (!strncmp("GenuineIntel", name, 12)) {
        switch (family) { // extract family code
        case 4:
            switch (model) { // extract model code
            case 0:
    ...
    else if (!strncmp("CyrixInstead", name, 12)) {
        strcpy_s(m_name, sizeof(PROC_CYRIX), PROC_CYRIX);
    }
    else if (!strncmp("CentaurHauls", name, 12)) {
...
notamd:
    ...
    if (pinfo) {
        ...
        strcpy_s(pinfo->name, MAX_VNAME_LENGTH + 1, Ident.cBuf);
